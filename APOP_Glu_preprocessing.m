...
outpath     = 'D:\Linus\APOP_Glu\preprocessed_BIDS';
...
for iFile = 1:numel(files)
    ...
    try
        ...
        % clean data
        % NOTE: WindowCriterionTolerances must be passed as a NUMERIC vector.
        % pop_clean_rawdata only str2num's it in its GUI branch, so a char string
        % like '[-Inf 7]' travels intact to clean_windows, where max('[-Inf 7]')
        % evaluates to 110 ('n') as the z-threshold and window rejection silently
        % never fires.
